...
def get_cosine_with_min_lr_schedule_with_warmup(optimizer,num_training_steps,warmap_ratio=0.01,cosine_ratio=0.9,min_lr_ratio=0.1):
    ...
    num_all_step=num_cosine_step+num_warmup_steps
    accelerator.print(num_warmup_steps,num_all_step,num_training_steps)
...
def init_model(lm_config):
    ...
    config = LMConfig.from_pretrained('out/')
    model = NanoChatLM.from_pretrained('out/', config=config)
...
#accelerate launch --config_file ds_zero2.yaml train_full_sft.py
# torchrun --nproc_per_node 2 train_full_sft.py
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NanoChat Full SFT")
    parser.add_argument("--out_dir", type=str, default="out_sft")
    ...
    parser.add_argument("--wandb_project", type=str, default="NanoChat-Full-SFT")
